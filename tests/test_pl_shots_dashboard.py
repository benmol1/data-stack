...
class TestTeamMetrics:
    ...
    def test_shots_per_match(self):
        rows = _make_shots(match_id="m1", home_team="A", away_team="B", side="h", n=5) + _make_shots(
            match_id="m1", home_team="A", away_team="B", side="a", n=3
        )
    ...
    def test_combined_is_sum_of_attack_and_defense_diff(self):
        ...
        for team in ["Alpha", "Beta"]:
            assert ts.loc[team, "combined"] == pytest.approx(ts.loc[team, "attack_diff"] + ts.loc[team, "defense_diff"])
...
class TestSeasonTable:
    ...
    def _two_match_rows(
        ...
        m1_home_result,
        m1_away_result,
        m2_home_result="SavedShot",
        m2_away_result="SavedShot",
    ):
    ...
    def test_win_gives_three_points_loss_gives_zero(self):
        ...
        st = _compute_season_table(self._two_match_rows("Goal", "SavedShot"))
    ...
    def test_draw_gives_one_point_each(self):
        ...
        st = _compute_season_table(self._two_match_rows("Goal", "Goal"))
...
def _compute_overlay_position(df, team):
    ...
    fixtures = (
        df.groupby(["match_id", "home_team", "away_team"]).size().reset_index()[["match_id", "home_team", "away_team"]]
    )
    ...
    mr["home_gd"] = mr["home_goals"] - mr["away_goals"]
    mr["away_gd"] = -mr["home_gd"]
    tbl = mr.groupby("home_team").agg(
        pts=("home_pts", "sum"), gd=("home_gd", "sum"), gf=("home_goals", "sum")
    ).rename_axis("team") + mr.groupby("away_team").agg(
        pts=("away_pts", "sum"), gd=("away_gd", "sum"), gf=("away_goals", "sum")
    ).rename_axis("team")
    tbl["position"] = (
        tbl[["pts", "gd", "gf"]].apply(lambda col: col.rank(method="min", ascending=False)).mean(axis=1).astype(int)
    )
